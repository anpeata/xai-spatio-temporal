...
\begin{frame}{Benchmark datasets (from literature)}
  \begin{columns}[T,onlytextwidth]
    \column{0.49\textwidth}
    \begin{block}{Large time-series archives}
      \begin{itemize}
        \item UCR (univariate) and UEA (multivariate) archives for reproducible baselines
        \item Useful for: scale, diversity, and statistical validation
      \end{itemize}
    \end{block}

    \begin{block}{Synthetic spatio-temporal}
      \begin{itemize}
        \item Controlled "moving cluster" simulations (known ground truth dynamics)
      \end{itemize}
    \end{block}

    \column{0.51\textwidth}
    \begin{block}{Real-world spatio-temporal datasets}
      \begin{itemize}
        \item Mobility trajectories (e.g., taxis, bike-sharing demand/flows)
        \item Environmental/health sensing (sensor networks, events in space+time)
      \end{itemize}
    \end{block}
  \end{columns}
