...
\begin{frame}{Workflow (from the task sketch)}
  \centering
  \begin{tikzpicture}[
    ...
    \node[box, right=of data2] (ts) {TS};
    \node[box, below=of ts] (shapelet) {Shapelet};

    % Repo grouping around TS + Shapelet (as in the sketch)
    \node[draw, rounded corners, inner sep=6pt, fit=(ts)(shapelet), label=above:{\small Repo}] (repo) {};

    \node[box, right=2.2cm of ts] (kmeans2) {KMeans};
    \node[box, right=of kmeans2] (shap2) {SHAP};
    \node[box, below=of shap2] (prod2) {Productive};

    \draw[arr] (data2.east) -- (ts.west);
    \draw[arr] (data2.east) |- (shapelet.west);

    \draw[arr] (ts.east) -- (kmeans2.west);
    \draw[arr] (shapelet.east) -| (kmeans2.west);

    \draw[arr] (kmeans2.east) -- (shap2.west);
    \draw[arr] (kmeans2.south) -- (prod2.north);
  \end{tikzpicture}
...
\begin{frame}{Implemented workflow (high-level)}
  ...
  \begin{tikzpicture}[
    ...
    \node[box, below=of kmeans, xshift=-2.05cm] (eval) {Evaluation\\Sil/CH/DB\\Stability (ARI)\\Post-hoc ARI/NMI};
    \node[box, below=of kmeans, xshift=2.05cm] (xai) {Surrogate tree\\+ SHAP};
    \node[box, below=of xai] (interp) {Interpretation\\plots + summaries};

    \draw[arr] (data) -- (features);
    \draw[arr] (features) -- (kmeans);
    \draw[arr] (kmeans.south west) -- (eval.north east);
    \draw[arr] (kmeans.south east) -- (xai.north west);
