...
\title[Weekly update]
{Progress update: Explainable clustering}
...
\begin{frame}{Suggested workflow}
  ...
  \begin{tikzpicture}[
    ...
    \draw[arr] (kmeans2.south) |- (prod2.west);
  \end{tikzpicture}
...
\begin{frame}{Unsupervised selection + evaluation (what is reported)}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{block}{Unsupervised $k$ choice (no labels)}
      ...
      \vspace{0.2em}
      {\scriptsize Internal indices reflect \textbf{geometric compactness/separation} in the chosen feature space (KMeans: Euclidean; roughly spherical clusters).}
    \end{block}
    ...
    \begin{block}{Post-hoc (labels only for eval)}
      ...
      \vspace{0.2em}
      {\scriptsize ARI/NMI compare two partitions; majority accuracy maps clusters$\rightarrow$labels by majority vote.}
    \end{block}
  \end{columns}
...
\begin{frame}{Benchmark datasets (from literature)}
  \footnotesize
  \begin{table}
    \centering
    \renewcommand{\arraystretch}{1.15}
    \begin{tabular}{@{}p{0.27\textwidth}p{0.40\textwidth}p{0.28\textwidth}@{}}
      \toprule
      \textbf{Category} & \textbf{Typical sources / examples} & \textbf{Why used} \\
      \midrule
      Large time-series archives & UCR (univariate), UEA (multivariate) & Reproducible baselines; scale/diversity; broad validation \\
      Real-world spatio-temporal & Mobility trajectories; bike-sharing demand/flows; sensor networks; environmental/health events & Domain realism; spatial + temporal structure; limited ground truth \\
      Synthetic spatio-temporal & Controlled moving-cluster simulations (known dynamics/labels) & Ground-truth evaluation; stress-test drift/merge/split behavior \\
      \bottomrule
    \end{tabular}
  \end{table}

  \vspace{0.25em}
  \footnotesize Goal: choose one public spatio-temporal benchmark (from the literature) as an intermediate validation before picoclimate windows.
\end{frame}
...
\begin{frame}{Next steps + questions (for today’s meeting)}
  ...
  \begin{itemize}
    ...
    \item Scale-up sanity check on a larger UCR dataset (e.g., \textbf{ECG5000}) before moving to picoclimate windows.
    \item Test on one public \textbf{spatio-temporal} benchmark dataset (selected from the literature).
  \end{itemize}
