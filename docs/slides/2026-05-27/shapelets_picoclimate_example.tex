...
\begin{columns}[T,onlytextwidth]
  ...
  \begin{tcolorbox}[enhanced,boxrule=1pt,arc=2pt,colback=clight,colframe=cmid]
    \scriptsize
    $x =$ [air\_temp\_s1, humidity\_s1, wind\_s1, ...,
    air\_temp\_s2, humidity\_s2, wind\_s2, ..., air\_temp\_s4, ...]
  \end{tcolorbox}
  \column{0.4\textwidth}
  \begin{itemize}\footnotesize\setlength\itemsep{4pt}
...
\begin{columns}[T,onlytextwidth]
  \column{0.6\textwidth}
  \begin{itemize}\footnotesize\setlength\itemsep{4pt}
    \item Candidate shapelets are sampled from mixed lengths (e.g., 5 and 10).
    \item Not all possible shapelets are enumerated; sampling is random.
...
\end{center}
\vspace{4pt}
{\scriptsize\itshape Input to KMeans is the fixed-width shapelet-distance matrix.}
\end{frame}
...
\begin{itemize}\footnotesize\setlength\itemsep{4pt}
  \item The current pipeline uses \textbf{all slot columns}, not one variable only.
  \item Mixed-length shapelets are sampled from the flattened slot vector.
  \item Distances become a fixed-width vector; that is the KMeans input.
  \item Using a single variable is possible, but changes the representation.
\end{itemize}
\end{frame}
