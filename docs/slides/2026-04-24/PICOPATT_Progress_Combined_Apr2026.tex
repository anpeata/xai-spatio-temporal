...
%   \leavevmode\hbox{%
%     \begin{beamercolorbox}[wd=0.72\paperwidth,ht=2.2ex,dp=1ex,leftskip=4pt]{normal text}%
%       \tiny\color{cmuted}An Pham \textbar{} LIRMM--ADVANSE \textbar{} Luc \& Reda%
%     \end{beamercolorbox}%
%     \begin{beamercolorbox}[wd=0.28\paperwidth,ht=2.2ex,dp=1ex,leftskip=10pt,left]{normal text}%
%       \scriptsize\color{cmuted}\insertframenumber/\inserttotalframenumber%
%     \end{beamercolorbox}}}
\setbeamertemplate{footline}{%
  \leavevmode\hbox{%
    % Increased width slightly for the name/info box
    \begin{beamercolorbox}[wd=0.85\paperwidth,ht=2.2ex,dp=1ex,leftskip=4pt]{normal text}%
      \tiny\color{cmuted}An Pham \textbar{} LIRMM--ADVANSE \textbar{} Luc \& Reda%
    \end{beamercolorbox}%
    % Decreased width and used 'right' alignment with 'rightskip'
...
\begin{columns}[T]
  ...
  \column{0.24\textwidth}\statcard{+17.6\%}{Silhouette gain\\ mixed vs fixed shapelets}{Consistent across\\ all 3 datasets}
   \column{0.24\textwidth}\statcard{6 seeds}{Explanation stability\\ quantified}{Top-5 feature overlap\\ 67--100\% across datasets}
...
% ── SLIDE 9 ──────────────────────────────────────────────────────────────────
\begin{frame}{Decision Points for Meeting}
\begin{columns}[T]
  \column{0.48\textwidth}
...
\begin{columns}[T,onlytextwidth]
  ...
  \begin{block}{Validated in code}
    ...
    \begin{itemize}\setlength\itemsep{1pt}
      ...
      \item Raw-vs-shapelet sanity check completed: mixed results by dataset
      \item HDBSCAN added to the picoclimate benchmark script
      \item Autoencoder + KMeans baseline rerun: silhouette 0.218 $\to$ 0.360 (test\_deep)
      \item Runtime audit added under \texttt{outputs/timings}: stability 14.3 min (quick) vs 119.3 min (full)
    \end{itemize}
    }
  \end{block}
...
\begin{columns}[T,onlytextwidth]
  ...
  \begin{block}{What this slide should show}
    \begin{itemize}
      \item Surrogate CV fidelity band across the 6-seed sweep
      \item Top-5 SHAP overlap across 6 seeds
      \item Runtime context: full stability sweep took about 119 minutes in the dated rerun
...
\begin{columns}[T,onlytextwidth]
  ...
  \begin{block}{Interpretation}
    \begin{itemize}
      ...
      \item Latest test\_deep rerun improves silhouette from 0.218 to 0.360
...
\begin{columns}[T,onlytextwidth]
  ...
  \begin{block}{Interpretation}
    \begin{itemize}
      ...
      \item Latest test\_benchmark run still favors KMeans (silhouette 0.218, k=2)
      \item HDBSCAN remains useful as the density/noise comparator in the matrix
...
\begin{columns}[T,onlytextwidth]
  ...
  \begin{block}{What is already supported}
    \begin{itemize}
      ...
      \item Shapelets help on some datasets, but the raw-vs-shapelet result is dataset-dependent.
    \end{itemize}
  \end{block}
