...
- per-city/per-track copies under `variable_slices/cities/<city>/<track_id>/` for browsing a single track at a time
...
- the `variable_slices/cities/` hierarchy is reconstructed from `variable_slices/` and does not depend on the root master tables
