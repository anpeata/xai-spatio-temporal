...
### 2026-04-27 (Repository housekeeping and slide rebuild)

**Experimentations**
- Audited local artifacts and redundant notebook files for cleanup.
- Recompiled the April slide deck from source to refresh the PDF.

**Results (numbers, tables, plots)**
- Removed redundant notebook: `scripts/notebooks/kmeans_test_temporal_shapelets_roma_taxi_minimal.ipynb`.
- Relocated summary CSV: `shapelet_results.csv` -> `docs/reports/shapelet_results.csv`.
- Rebuilt slide PDF from `docs/slides/2026-04-24/PICOPATT_Progress_Combined_Apr2026.tex`.
- New PDF artifact: `docs/slides/2026-04-24/PICOPATT_Progress_Combined_Apr2026.pdf` (17 pages, 520078 bytes, rebuilt on 2026-04-27).

**Insights**
- The minimal Roma shapelet notebook contained summary text only and was safe to remove after reference checks.
- Keeping report CSV artifacts under `docs/reports/` is cleaner than storing them at repo root.

**Failures / issues / risks**
- None in this cleanup pass.

**Implementation details**
- Root LaTeX byproducts (`.aux/.log/.out`) generated by ad-hoc builds were removed locally.
- Deck was compiled with two `pdflatex` passes from the slide directory.

**Next**
- Keep future slide builds in-place under `docs/slides/...` and avoid root-level TeX artifacts.
- Keep summary report tables in `docs/reports/` for easier auditing.
...
- Confirm any corrections to the mixed-length pipeline map if needed.
