...
def run_summary(config: SummaryConfig, data_path: Path) -> Dict[str, object]:
    ...
    km_sil = _safe_silhouette(x_shapelet, km_labels)

    ward_labels = AgglomerativeClustering(n_clusters=best_k).fit_predict(x_shapelet)
    ward_sil = _safe_silhouette(x_shapelet, ward_labels)

    hdb_labels, hdb_summary = _hdbscan_summary(x_shapelet)

    explain_cols = [c for c in df.columns if c.endswith("__mean") or c.endswith("__std") or c.endswith("__trend")]
    explain_df = df[explain_cols].copy()
    ...
    if hdb_summary is not None:
        ...
        top = _top_features(explain_df, hdb_labels, explain_cols, config.top_n)
...
def main() -> None:
    ...
    summary = run_summary(config, args.data_path)
